...
def create_nuscenes_infos(root_path,
    ...
    # TODO remove rosbag for using various scenes
    # available_scene_names = ['scene-0061', 'scene-0553', 'scene-0655', 'scene-0757', 'scene-0796', 'scene-1077', 'scene-1094', 'scene-1100']
    train_scenes = ['rosbag']
    val_scenes = ['rosbag']
    train_scenes = list(
...
def _fill_trainval_infos(nusc,
    ...
    for sample in mmcv.track_iter_progress(nusc.sample):
        ...
        for cam in camera_types:
            if len(sample['data']) != len(camera_types) + 1:
                print("Missing camera / lidar types: ", list(sample['data'].keys()))
                continue
...
def export_2d_annotation(root_path, info_path, version, mono3d=True):
    ...
    for info in mmcv.track_iter_progress(nusc_infos):
        for cam in camera_types:
            if len(info['cams']) != len(camera_types):
                print("Info['cams'] and camera types have different size - ",len(info['cams']))
                continue
